...
- `Login.vue` 去除身份下拉框，通过真实账号密码匹配登录，支持返回 `redirect`；`Register.vue` 提供普通学生注册。
...
7. **重构动态地点选择器**：AI 协助重构了地点选择器组件，实现了根据用户选择动态改变下级表单字段的复杂前端联动交互——封装独立组件 `LocationSelector.vue`，根据建筑类型（宿舍楼需选楼栋+楼层、教学楼/食堂只需选楼层、精弘桥/操场等地标直接填详细地址）动态渲染字段，并自动拼接完整地址。
8. **用户认证体系与角色分配闭环**：AI 协助重构了登录页，**移除了身份选择**，改为真实账号密码匹配登录；**创建了注册页（默认学生 `role = student`）**；新增内置系统管理员 `sysadmin`；并实现了系统管理员在【账号管理】中**动态分配角色与禁用账号**的完整 RBAC 闭环，用户数据持久化到 `localStorage`。
